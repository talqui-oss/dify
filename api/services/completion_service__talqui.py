...
class CompletionService:
    ...
    # TALQUI IMPLEMENTATION
    @classmethod
    def talqui_completion(cls, app_id: str, app_model: App, user: Union[Account | EndUser], args: Any,
        ...
        if conversation_id:
            ...
            if not conversation:
                ...
                status='normal',
                from_source='api',
                from_end_user_id=args['contactID'],
                from_account_id=None,
            )
            db.session.add(conversation)
            
        logging.info(app_model_config.__dict__)


        for message in history:
            existing_message = db.session.query(Message).filter_by(id=message['messageID']).first()
            
            # UPSERTING EXTERNAL MESSAGE IN HISTORY
            if existing_message:
                logging.info("@compeltion:service:talqui - message already exists")
            else: 
                logging.info("@compeltion:service:talqui - inserting new external message")
                message = Message(
                    id=message['messageID'],
                    app_id=app_model.id,
                    model_provider=app_model_config.provider,
